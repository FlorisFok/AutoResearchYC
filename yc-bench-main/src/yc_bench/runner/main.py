...
def _build_db_url(args, episode: int, max_episodes: int) -> str:
    ...
    config_label = Path(args.config_name).stem if "/" in args.config_name or args.config_name.endswith(".toml") else args.config_name
    db_dir = Path("db")
    db_dir.mkdir(exist_ok=True)
    base = f"{config_label}_{args.seed}_{slug}"
    if max_episodes > 1:
...
def run_benchmark(args):
    ...
    for episode in range(1, max_episodes + 1):
        ...
        _config_label = Path(args.config_name).stem if "/" in args.config_name or args.config_name.endswith(".toml") else args.config_name
        transcript_path = Path("db") / f"{_config_label}_{args.seed}_{_slug}.transcript.jsonl"
        session_messages_path = Path("db") / f"{_config_label}_{args.seed}_{_slug}.session.json"
    ...
    results_path = results_dir / f"yc_bench_result_{config_label}_{args.seed}_{slug}.json"
